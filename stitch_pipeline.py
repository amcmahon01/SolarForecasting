...
@ray.remote
class Stitch(object):
    def __init__(self, t=None, img_obs=None, stitch_path=None, height_group=None):

        self.btime = t
        self.stitch_path = stitch_path
        self.height_group = height_group                                        
        ...
        if img_obs is not None:
            self.img_obs = {cid:ray.get(f_obs) for (cid, f_curr), f_obs in img_obs.items()} # Simplify key and retrieve objects
    ...
    def generate_stitch(self):
        ...
        try:
            img_obs = self.img_obs  # Use preloaded data
            ...
            for ilayer, height in enumerate(h):
                ...
                if nstch_x > 10000 or nstch_y > 10000:
                    logging.warning("Stitch area too large to process, likely miscalculated cloud height on rainy day.  Cloud height(m): %i, nstch_x: %i, nstch_y: %i" % (height, nstch_x, nstch_y))
                    return False
                ...
                for i, img in enumerate(imgs):
                    ...
                    # '==1' conditions added here to deal with netcdf files with undefined fill values
                    if img.cloud_mask is not None:
                        tmp = (np.flip(img.cloud_mask, axis=1)==1)  # tmp[img.cm!=ilayer+1,:]=0;
                        msk[start_y:start_y + img.camera.ny, start_x:start_x + img.camera.nx][mk] += tmp[mk]

                    if img.bright_mask is not None:
                        tmp = (np.flip(img.bright_mask, axis=1)==1);  # tmp[img.cm!=ilayer+1,:]=0;
    ...
    """
    @ray.method(num_returns=1)
    def process_cloud_heights(self):
        r_sum = 0
        ...
        for cid, f_obs in img_obs.items():
            ...
            #results = [self.compute_cloud_height(ray.get(img_obs[cid]), catch(lambda:ray.get(img_obs[cam_neigh]))) for cam_neigh in self.height_group[cid]]
            results = [self.compute_cloud_height(img_obs[cid], catch(lambda:img_obs[cam_neigh])) for cam_neigh in self.height_group[cid]]
    ...
    def compute_cloud_height(self, base_img, img_neig, layer=0, distance=None):
        ...
        try:
            ...
            logging.info("Missing attribute, skipping: " + str(e))
            return False
...
def restore_ncdf(filename):
    ...
    logging.info("\tReading stitch file " + str(filename))
    try:

        root_grp = nc4.Dataset(filename, 'r', format='NETCDF4')  # 'r' stands for read

        stch = StitchShare(filename[-17:-3])
        ...
        logging.warning("\tAn error occurred reading the NetCDF file " + str(filename) + ": " + str(e))
        return None
